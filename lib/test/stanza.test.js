...
function traverseObject(obj) {
    for (const key of Object.keys(obj)) {
        console.log(key);
        if (obj[key] instanceof Element && parseFuncMap.has(obj[key].namespaceURI)) {
            console.log(obj[key].namespaceURI);
            // console.log(xmlSerializer.serializeToString(obj[key]));
            const a = parseFuncMap.get(obj[key].namespaceURI)(obj[key]);
            // console.log(a);
            traverseObject(a);
            obj[key] = a;
        }
...
describe('消息解析器', () => {
    ...
    it('应该解析包含主题和正文的消息', () => {
        ...
        const result = Message.parseMessage(message);
    ...
    it("正确解析carbon消息", () => {
        ...
        const result = traverseObject(msgEl);
